...
#[derive(Debug, Error)]
pub enum GeminiRunnerError {
    #[error(
        "gemini CLI를 실행할 수 없습니다. 먼저 `gemini` 1회 실행으로 Google 계정 로그인을 완료하고 PATH를 확인하세요. 원인: {0}"
    )]
    Unavailable(#[source] io::Error),
    #[error(
        "gemini CLI 호출이 실패했습니다. 먼저 `gemini` 실행으로 로그인 상태를 확인하세요. 상세: {0}"
    )]
    Invocation(String),
    #[error(
        "gemini CLI 응답이 {0} 안에 완료되지 않았습니다. 먼저 `gemini` 상태와 네트워크를 확인하세요."
    )]
    Timeout(String),
    #[error("gemini CLI가 빈 응답을 반환했습니다. 먼저 `gemini` 로그인 상태를 확인하세요.")]
    EmptyResponse,
}
...
impl GeminiRunner {
    ...
    pub fn healthcheck(&self) -> Result<bool> {
        match Command::new(&self.command).arg("--version").output() {
            ...
            Err(error) => Err(GeminiRunnerError::Unavailable(error).into()),
        }
    }
        ...
        match result {
            Ok(outcome) if outcome.success => {
                ...
                if response.is_empty() {
                    ...
                    return Err(GeminiRunnerError::EmptyResponse.into());
                }
            ...
            Ok(outcome) => {
                let detail = if outcome.stderr.is_empty() {
                    if outcome.stdout.is_empty() {
                        "gemini exited with a non-zero status".to_string()
                    } else {
                        outcome.stdout
                    }
                } else {
                ...
                Err(GeminiRunnerError::Invocation(detail).into())
            }
            Err(SubprocessError::Spawn(error)) | Err(SubprocessError::Wait(error)) => {
                ...
                Err(GeminiRunnerError::Unavailable(error).into())
            }
            Err(SubprocessError::Timeout) => {
                ...
                Err(GeminiRunnerError::Timeout(format_timeout(self.timeout)).into())
            }
        }
    }
...
impl PromptRunner for GeminiRunner {
    fn run_prompt_named(&self, label: &str, prompt: &str) -> Result<String> {
        GeminiRunner::run_prompt_named(self, label, prompt)
    }
}
