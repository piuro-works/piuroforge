...
Rust로 구현된 CLI-first AI 소설 작성 프레임워크를 제공한다. 사용자는 CLI 명령만으로 소설 생성, 리뷰, 수정, 승인, 상태 확인을 수행한다.
...
- 사용자 전역 설정: `~/.config/piuroforge/config.toml`
...
`piuroforge init`은 누락된 필드를 인터랙티브하게 질문할 수 있어야 한다.
`piuroforge next-scene`은 위 필드가 비어 있으면 실패해야 한다.
기본 bundle 정책은 `bundle_scene_target = 3`이며, 기본 scene 역할은 `incident -> escalation -> cliffhanger`다.
프로젝트 차원의 문체 제어는 named-author imitation이 아니라 `03_StoryBible/Voice/` 아래의 style/tone/genre/voice guide 문서로 수행한다.

## Required Commands

- `piuroforge capabilities`
- `piuroforge init [PATH]`
- `piuroforge status`
- `piuroforge doctor`
- `piuroforge next-scene`
- `piuroforge review`
- `piuroforge rewrite SCENE_ID --instruction "..."`
- `piuroforge approve SCENE_ID`
- `piuroforge next-bundle`
- `piuroforge expand-world`
- `piuroforge memory`
- `piuroforge show SCENE_ID`

## CLI Output Contract

- 기본 출력은 사람이 읽기 쉬운 `text` 모드다.
- 모든 명령은 `--format json`을 지원해야 한다.
- 모든 JSON 응답은 `schema_version`과 `agent_mode`를 포함해야 한다.
- 모든 명령은 `--agent`를 지원해야 하며, text 모드에서는 compact key-value 출력으로 축약되어야 한다.
- JSON 출력은 최소한 다음 필드를 포함해야 한다.
  - 성공: `schema_version`, `status`, `agent_mode`, `command`, `workspace`, `summary`, `details`, `artifacts`, `next_steps`
  - 실패: `schema_version`, `status`, `agent_mode`, `command`, `error_code`, `reason`, `remediation`
- command별 추가 machine payload가 필요하면 선택 필드 `data`를 사용할 수 있다.
- `piuroforge init --format json`은 interactive prompt를 기다리지 않고 non-interactive로 동작해야 한다.
- `--help`에는 사용 예시와 JSON 자동화 예시가 포함되어야 한다.

## Distribution Contract

- 저장소 루트에 `install.sh`를 둔다.
- 사용자는 `curl -fsSL https://raw.githubusercontent.com/piuro-works/piuroforge/main/install.sh | bash` 형태로 설치할 수 있어야 한다.
- `install.sh`는 GitHub Releases에서 현재 플랫폼에 맞는 `piuroforge-<target>.tar.gz` 자산을 내려받아 설치한다.
...
- opt-in dummy fallback:
  - 기본값은 비활성화다.
  - 전역 설정 `allow_dummy_fallback = true` 또는 `PIUROFORGE_ALLOW_DUMMY=true`일 때만 허용한다.
...
- draft scene 파일: `02_Draft/Scenes/scene_<bundle>_<scene>-<slug>.md`를 기본으로 하되, slug는 `short_title` 기준이어야 한다. slug를 만들 수 없으면 `scene_<bundle>_<scene>.md`도 허용한다.
- draft bundle 파일: `02_Draft/Bundles/bundle_<bundle>-<slug>.md`를 기본으로 하되, slug는 compiled bundle `short_title` 기준이어야 한다. slug를 만들 수 없으면 `bundle_<bundle>.md`도 허용한다.
- story bible 폴더:
...
- rewrite metadata record는 workspace root 기준 상대경로와 `source_review_score`, `post_rewrite_review_score`를 저장해야 한다.
...
2. planner가 short_title/bundle_role/goal/conflict/outcome을 만든다.
...
4. 가장 최근 rewrite metadata가 있으면 `post_rewrite_review_score`를 backfill한다.
5. fallback이 개입된 경우 warning과 함께 `ReviewOutcome`을 반환한다.

### rewrite_scene

1. 대상 scene을 로드한다.
2. 있으면 직전 review score를 읽는다.
3. 기존 scene 원본 snapshot을 저장한다.
4. instruction을 반영해 editor 또는 writer를 다시 실행한다.
5. 수정본 snapshot과 metadata를 저장한다.
6. rewrite metadata에는 `source_review_score`를 기록하고, `post_rewrite_review_score`는 다음 review 전까지 `null`로 둔다.
7. 수정된 text를 현재 scene 파일에 반영한다.
...
1. 현재 bundle의 scene markdown를 모은다.
2. scene 번호가 1부터 연속인지 검증한다.
3. bundle의 scene 개수가 `bundle_scene_target`과 정확히 일치하는지 검증한다.
4. bundle markdown를 생성한다.
