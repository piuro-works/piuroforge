...
기본 chapter 정책은 `chapter_scene_target = 3`이며, 기본 scene 역할은 `incident -> escalation -> cliffhanger`다.
...
2. planner가 short_title/chapter_role/goal/conflict/outcome을 만든다.
...
3. chapter의 scene 개수가 `chapter_scene_target`과 정확히 일치하는지 검증한다.
4. chapter markdown를 생성한다.
