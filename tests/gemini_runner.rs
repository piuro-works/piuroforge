use anyhow::Result;
use piuroforge::gemini_runner::GeminiRunner;
...
fn run_prompt_returns_stdout_from_subprocess() -> Result<()> {
    ...
    let script_path = temp_dir.path().join("fake_gemini.sh");
    let script = "#!/bin/sh\n# fake gemini: echo back a fixed reply, ignoring args.\nprintf 'gemini ok\\n'\n";
    std::fs::write(&script_path, script)?;
    let mut permissions = std::fs::metadata(&script_path)?.permissions();
    permissions.set_mode(0o755);
    std::fs::set_permissions(&script_path, permissions)?;

    let runner = GeminiRunner::new(script_path.display().to_string(), Duration::from_secs(5));
    let response = runner.run_prompt("hello")?;

    assert_eq!(response, "gemini ok");
...
fn run_prompt_times_out_when_command_hangs() -> Result<()> {
    let temp_dir = tempdir()?;
    let script_path = temp_dir.path().join("hanging_gemini.sh");
    let script = "#!/bin/sh\nsleep 5\nprintf 'too late\\n'\n";
    std::fs::write(&script_path, script)?;
    let mut permissions = std::fs::metadata(&script_path)?.permissions();
    permissions.set_mode(0o755);
...
fn run_prompt_writes_opt_in_prompt_log() -> Result<()> {
    let temp_dir = tempdir()?;
    let script_path = temp_dir.path().join("logging_gemini.sh");
...
#[test]
fn run_prompt_returns_invocation_error_when_subprocess_fails() -> Result<()> {
    let temp_dir = tempdir()?;
    let script_path = temp_dir.path().join("failing_gemini.sh");
    ...
    let error = runner
        .run_prompt("hello")
        .expect_err("expected invocation error for failing command");

    assert!(error.to_string().contains("호출이 실패"));
